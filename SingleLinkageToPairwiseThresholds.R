...
####################################################################################
####### Automatic SingleLinkageToPairwiseThresholds.R script with Rscript ##########
...
if (length(args)!=7) {
  stop("Please, provide arguments below\n
       USAGE: Rscript SingleLinkageToPairwiseThresholds.R DFP SLA SLB SLC SLD MNS PSP
       - DFP: dataframe of profiles
       - SLA: single linkage threshold A
       - SLB: single linkage threshold B
       - SLC: single linkage threshold C
       - SLD: single linkage threshold D
       - MNS: minimum number of samples per cluster
       - PSP: splitting for the optimization samples (%)
       NB: SLA < SLB < SLC < SLD
       e.g. Rscript SingleLinkageToPairwiseThresholds.R Profiles.csv 4 7 10 13 6 50", call.=FALSE)
}
# test if arguments are well provided
## args[1]
if (grepl("\\.csv$", args[1])==FALSE) {
  stop("The argument DFP (dataframe of profiles) must be a comma-separated values file (.csv)\n", call.=FALSE)
}
## args[2]
if (!grepl("\\D",args[2])==FALSE) {
  stop("The argument SLA (single linkage threshold A) must be a number\n", call.=FALSE)
}
## args[3]
if (!grepl("\\D",args[3])==FALSE) {
  stop("The argument SLB (single linkage threshold B) must be a number\n", call.=FALSE)
}
## args[4]
if (!grepl("\\D",args[5])==FALSE) {
  stop("The argument SLC (single linkage threshold C) must be a number\n", call.=FALSE)
}
## args[5]
if (!grepl("\\D",args[5])==FALSE) {
  stop("The argument SLD (single linkage threshold D) must be a number\n", call.=FALSE)
}
## args[6]
if (!grepl("\\D",args[6])==FALSE) {
  stop("The argument MNS (minimum number of samples per cluster) must be a number\n", call.=FALSE)
}
## args[7]
if (!grepl("\\D",args[7])==FALSE) {
  stop("The argument PSP (splitting for the optimization samples) must be a number\n", call.=FALSE)
}
if (((as.numeric(args[7]) <= 0) || (as.numeric(args[7]) >= 100)) == TRUE) {
  stop("The argument PSP (splitting for the optimization samples) must be between 0 and 100\n", call.=FALSE)
}
...
# define arguments
## argDFP: dataframe of profiles (Objective 1)
argDFP <- read.table(args[1], dec = ".", header=TRUE, sep = ",", quote = "")
## argSLA: single linkage threshold A (Objective 1)
argSLA <- args[2]
argSLA <- as.numeric(argSLA)
## argSLB: single linkage threshold B (Objective 1)
argSLB <- args[3]
argSLB <- as.numeric(argSLB)
## argSLC: single linkage threshold C (Objective 1)
argSLC <- args[4]
argSLC <- as.numeric(argSLC)
## argSLD: single linkage threshold D (Objective 1)
argSLD <- args[5]
argSLD <- as.numeric(argSLD)
## argMNS: minimum number of samples to retain inside each single linkage cluster (Objective 1)
argMNS <- args[6]
argMNS <- as.numeric(argMNS)
## argPSP: proportion of splitting (%) defining the optimization dataframes (Objective 2)
argPSP <- args[7]
argPSP <- as.numeric(argPSP)

## print arguments
cat("argument DFP:",args[1],"\n")
cat("argument SLA:",args[2],"\n")
cat("argument SLB:",args[3],"\n")
cat("argument SLC:",args[4],"\n")
cat("argument SLD:",args[5],"\n")
cat("argument MNS:",args[6],"\n")
cat("argument PSN:",args[7],"\n")
...
### plot
if(!interactive()) pdf(NULL) ## suppress Rplots.pdf generation
...
p = ggplot(SLPWcorrelationIntramax, aes(x = numSLthreshold, y = PWdifferences)) +
  ...
  geom_point(shape=18, color = "#000000") +
  geom_smooth(size = 0.5, color="#000000", fill = "#A9A9A9", method=lm, linetype="dashed", se=TRUE, formula = my.formula) +
  ggtitle("Correlation between \n maximum pairwise differences of single linkage clusters \n and single linkage thresholds \n ") +
...
# add message
cat("Developped by Nicolas Radomski on February 14 (2022) with the R version 4.1.2 (2021-11-01)","\n")
